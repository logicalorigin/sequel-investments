...
export const loanApplicationsRelations = relations(loanApplications, ({ one, many }) => ({
  ...
  broker: one(brokerProfiles, {
    fields: [loanApplications.brokerId],
...
// Broker KYC status enum
export const brokerKycStatusEnum = pgEnum("broker_kyc_status", ["pending", "approved", "rejected", "suspended"]);

// Broker invite status enum (for borrower invites from brokers)
export const brokerInviteStatusEnum = pgEnum("broker_invite_status", ["pending", "accepted", "expired", "revoked"]);
...
export const brokerProfiles = pgTable("broker_profiles", {
  ...
  // Company Info
  companyName: text("company_name").notNull(),
  companySlug: varchar("company_slug").notNull().unique(), // Used for white-label subdomain
  nmlsNumber: text("nmls_number"),
  ...
  // Settings
  defaultCommissionBps: integer("default_commission_bps").default(100), // Basis points (100 = 1%)
...
export const brokerProfilesRelations = relations(brokerProfiles, ({ one, many }) => ({
  ...
  branding: one(brokerBranding),
  borrowers: many(brokerBorrowers),
  invites: many(brokerInvites),
}));
...
// Broker Branding table - White-label customization
export const brokerBranding = pgTable("broker_branding", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  brokerProfileId: varchar("broker_profile_id").notNull().references(() => brokerProfiles.id).unique(),
  
  // Logo & Images
  logoUrl: text("logo_url"),
  logoObjectKey: text("logo_object_key"), // For internal storage
  faviconUrl: text("favicon_url"),
  faviconObjectKey: text("favicon_object_key"),
  
  // Color Scheme (HSL format for CSS variables)
  primaryColor: text("primary_color").default("222 47% 31%"), // Dark blue
  secondaryColor: text("secondary_color").default("171 77% 36%"), // Teal/mint
  accentColor: text("accent_color").default("171 77% 36%"),
...
export const brokerBrandingRelations = relations(brokerBranding, ({ one }) => ({
  brokerProfile: one(brokerProfiles, {
    fields: [brokerBranding.brokerProfileId],
    references: [brokerProfiles.id],
  }),
}));

export type BrokerBranding = typeof brokerBranding.$inferSelect;
export type InsertBrokerBranding = typeof brokerBranding.$inferInsert;

export const insertBrokerBrandingSchema = createInsertSchema(brokerBranding).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

// Broker-Borrower relationship table - Links brokers with their retail clients
export const brokerBorrowers = pgTable("broker_borrowers", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  brokerId: varchar("broker_id").notNull().references(() => brokerProfiles.id),
  borrowerId: varchar("borrower_id").notNull().references(() => users.id),
  
  // Relationship Details
  status: brokerBorrowerStatusEnum("status").default("active").notNull(),
  isPrimaryBroker: boolean("is_primary_broker").default(true).notNull(),
  
  // How they connected
  referralSource: text("referral_source"), // "invite", "manual", "transfer"
  inviteId: varchar("invite_id").references(() => brokerInvites.id),
  ...
  index("idx_broker_borrowers_broker").on(table.brokerId),
  index("idx_broker_borrowers_borrower").on(table.borrowerId),
]);

export const brokerBorrowersRelations = relations(brokerBorrowers, ({ one }) => ({
  broker: one(brokerProfiles, {
    fields: [brokerBorrowers.brokerId],
    references: [brokerProfiles.id],
  }),
  borrower: one(users, {
    fields: [brokerBorrowers.borrowerId],
    references: [users.id],
  }),
  invite: one(brokerInvites, {
    fields: [brokerBorrowers.inviteId],
    references: [brokerInvites.id],
  }),
}));

export type BrokerBorrower = typeof brokerBorrowers.$inferSelect;
export type InsertBrokerBorrower = typeof brokerBorrowers.$inferInsert;

export const insertBrokerBorrowerSchema = createInsertSchema(brokerBorrowers).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

// Broker Invites table - For sending portal invites to new retail clients
export const brokerInvites = pgTable("broker_invites", {
  ...
  // Token & Status
  token: varchar("token").notNull().unique(),
  status: brokerInviteStatusEnum("status").default("pending").notNull(),
  ...
  // Optional: Pre-fill application data
  prefilledData: jsonb("prefilled_data"), // Loan type, property address, etc.
  
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("idx_broker_invites_broker").on(table.brokerId),
  index("idx_broker_invites_token").on(table.token),
]);

export const brokerInvitesRelations = relations(brokerInvites, ({ one }) => ({
  broker: one(brokerProfiles, {
    fields: [brokerInvites.brokerId],
    references: [brokerProfiles.id],
  }),
  acceptedByUser: one(users, {
    fields: [brokerInvites.acceptedByUserId],
    references: [users.id],
  }),
}));

export type BrokerInvite = typeof brokerInvites.$inferSelect;
export type InsertBrokerInvite = typeof brokerInvites.$inferInsert;

export const insertBrokerInviteSchema = createInsertSchema(brokerInvites).omit({
  id: true,
  createdAt: true,
  acceptedAt: true,
  acceptedByUserId: true,
