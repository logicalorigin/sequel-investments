...
export default function LoginPage() {
  ...
  const devLoginMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/borrower/dev-login", {});
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/auth/user"] });
      toast({
        ...
        description: "Logged in as test borrower.",
      });
      setLocation("/portal");
    },
    onError: (error: any) => {
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button 
              type="submit" 
              className="w-full" 
              disabled={loginMutation.isPending || devLoginMutation.isPending}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-card px-2 text-muted-foreground">Testing Only</span>
            </div>
          </div>
